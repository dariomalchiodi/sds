...
approfondita nei Capitoli {numref}`%s <sec:stime-puntuali>` e
{numref}`%s <sec:stime-per-intervalli>`, mentre la seconda viene accennata
nel {numref}`Capitolo %s <cap:statistica-non-parametrica>`.
...
rispettivamente nei Capitoli {numref}`%s <sec:stime-puntuali>` e
{numref}`%s <sec:stime-per-intervalli>`.
...
{numref}`Paragrafo %s <sec:intervalli-fiduciari>`.
```
...
{numref}`Paragrafo %s <sec:intervalli-fiduciari>`, il suo valore non indica
