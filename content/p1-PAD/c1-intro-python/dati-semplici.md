...
```{margin}
Nel Paragrafo @attributi vedremo che il _tipo di un attributo_ all'interno di
un _dataset_ è un concetto diverso rispetto al _tipo di dato_ che corrisponde
a come vengono memorizzati i valori dell'attributo stesso.
```
...
l'obbligo di rendere esplicite le dichiarazioni [^type-hinting] e affidandosi a un _type checking dinamico_
...
Ciò significa quindi che tecnicamente non ha più senso parlare del tipo di una
variabile, o di un parametro, o del valore restituito da una funzione:
quest'ultima potrebbe ad esempio restituire tipi differenti a seconda dei
parametri attuali specificati durante la sua invocazione; analogamente, è
possibile assegnare a una variabile un valore il cui tipo è completamente
diverso da quello del valore precedentemente memorizzato, e così via. In
sintesi, le operazioni che è possibile effettuare su una variabile, su un
parametro e così via dipendono dal ciò che essi contengono in un dato momento
[^duck-typing]. La definizione del concetto stesso di _tipo_ diventa pertanto
in un certo senso indiretta: nei linguaggi con _tipizzazione statica_ (come
C, o Java, o Go) è necessario dichiarare esplicitamente i tipi, e la
dichiarazione determina le operazioni che si possono effettuare. Per esempio,
su un oggetto di tipo `Anatra` sarà possibile invocare i metodi `cammina`,
`nuota` e `starnazza`. In Python, invece, la tipizzazione è dinamica e si
assiste a una sorta di rovesciamento del concetto di _semantica_: sono le
operazioni eseguibili su un oggetto in un dato momento della sua vita a
determinarne il tipo. Pertanto, se è possibile invocare i metodi `cammina`,
`nuota` e `starnazza` su un oggetto, diremo che questo è equivalente a
un'`Anatra`[^duck-typing].
...
una funzione, o il valore da essa restituito). Va sottlineato che la tipizzazione resta comunque
...
il Capitolo @cap:dati-strutturati si concentrerà sui principali
...
[^duck-typing]: Questo esempio non è fatto a caso: si utilizza infatti proprio
