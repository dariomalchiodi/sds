...
title: 2.1. Installazione, configurazione e primi passi
---
...
In tutti gli esempi che seguono, il simbolo `$` indica il _prompt_ di una
_shell_, che nel sistema che usate potrebbe essere configurata per mostrare
un _prompt_ di tipo diverso. Solo nell'esempio qui sotto, invece,
`my_parent_dir` è un nome fittizio da sostituire con il _pathname_ della
...
In tutti gli esempi che seguono, la dicitura `C:>` indica il _prompt_ di una
_PowerShell_, che nel sistema che usate potrebbe essere configurata per
mostrare un _prompt_ diverso. Solo nell'esempio qui sotto, invece,
`my_parent_dir` è un nome fittizio da sostituire con il _pathname_ della
directory nella quale si vogliono posizionare i contenuti del libro.
...
Nel seguito del paragrafo supporrò che la _PowerShell_ nella quale sono stati
eseguiti questi comandi risulti ancora in esecuzione.
````
`````
...
[Approccio](approccio). Infine, imparare a usare git è una cosa che raccomando
a chiunque approcci lo studio non solo dell'informatica, ma anche di tutte le
discipline che in qualche modo afferiscono alla _data science_. Di fatto, git è
utilizzato per gestire la stragrande maggioranza dei progetti software, dunque
vale la pena imparare a usarlo fin da subito.
...
Paragrafo [Uno sguardo di insieme](uno-sguardo-di-insieme) per produrre grafici
...
:sync: tab-linux
```bash
(sds) $ myst start --execute
```
````
````{tab-item} Windows
:sync: tab-win
```cmd-prompt
(sds) C:> myst start --execute
```
````
`````

che eseguirà un server web locale (in ascolto per _default_ sulla porta `3000`),
al quale collegarsi usando un browser per leggere il libro nella stessa forma
che ha sul relativo sito web.
