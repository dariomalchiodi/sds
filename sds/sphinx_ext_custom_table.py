...
class CustomTableDirective(SphinxDirective):
    ...
    def run(self):
        ...
        if 'width' in self.options:
            table_node['width'] = self.options['width']
        ...
        table_node.extend(temp_node.children)
        if caption_node:
            table_node += caption_node

        if 'name' in self.options:
            name = self.options['name']
            table_node['ids'].append(name)
            table_node['names'].append(name)

            # Only register if not already in domain
            domain = self.env.get_domain('std')
            if name not in domain.labels:
                self.state.document.note_explicit_target(table_node)
...
def setup(app):
    app.add_directive("customtable", CustomTableDirective)
    return {
        'version': '0.1',
